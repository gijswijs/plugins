...
def on_htlc_accepted(htlc, onion, plugin, request, **kwargs):
    ...
    # Check if the peer supports PSS routing
    if not int(peer['features'], 16) & (1 << PSS_FEATURE):
        plugin.log("Peer {} node doesn't support PSS routing. Features: {}".format(
            peer['id'], peer['features']))
        request.set_result({"result": "continue"})
        return
...
def init(options, configuration, plugin: Plugin):
    plugin.log("pss.py initializing {}".format(configuration))
    plugin.node_id = plugin.rpc.getinfo()['id']

    plugin.try_timeout = int(options.get("pss-try-timeout"))
