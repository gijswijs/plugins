...
def init(options, configuration, plugin: Plugin):
    ...
    # Set of currently active split payments, keyed by their payment_hash
    plugin.splits = {}

def send_msg(plugin: Plugin, peer_id, msgtype, contents):
    msg = (msgtype.to_bytes(2, 'big')
           + bytes(contents, encoding='utf8'))
    plugin.rpc.sendcustommsg({'node_id': peer_id, 'msg': msg.hex()})
...
def on_custommsg(peer_id, payload, plugin, request, **kwargs):
    ...
    if mtype == MSGTYPE_SPLIT_PAYMENT:
        plugin.log("Received message about split payment to be received with payment hash {}".format(data))
...
def on_htlc_accepted(htlc, onion, plugin, request, **kwargs):
    ...
    plugin.log("Payload disected {}, {}, {}".format(amt_to_forward, outgoing_cltv_value.value, short_channel_id.value))

    adjusted_payload = TlvPayload()

    adjusted_amt = int(amt_to_forward / 2)

    adjusted_payload.add_field(2, adjusted_amt.to_bytes(2, 'big'))
    ...
    plugin.log("Found peer and channel {}, {}, for scid {}".format(peer, chan, str(scid)))
    ...
    send_msg(plugin, peer['id'], MSGTYPE_SPLIT_PAYMENT, htlc['payment_hash'])

    plugin.log("Adjusted payload: {}".format(adjusted_payload.to_hex()[2:]))

    request.set_result({"payload": adjusted_payload.to_hex()[2:], "result": "continue"})
